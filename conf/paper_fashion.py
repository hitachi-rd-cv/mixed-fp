...
def get_config():
    ...
    # scale = 2e-2
    #scale = 5e-2
    option_dataset = {'scale_noise': 0.0, 'scaler': 'standard_flat'}
    option_model = {'scale_reg': 1e-1}
    device = 'cuda'
    name_hyper_model = "influence"
    dtype = 'float'
    kwargs_method = {}
    option_oracle = {}
    d_configs = {}
    ...
    for a in [1e-5, 2e-5, 5e-5, 1e-4, 2e-4, 5e-4, 1e-3, 2e-3, 5e-3, 1e-2, 2e-2, 5e-2]:
        d_configs.update({f'vr_no_km(a={a})': ('vr', {'a': a, 'scale': scale})})

    scheduler = 'const'
    for lr_ in [1e-3, 2e-3, 5e-3, 1e-2, 2e-2, 5e-2, 1e-1, 2e-1, 5e-1, 1e0]:
        d_configs.update({f'grazzi_{scheduler}(lr={lr_})': ('grazzi', {'lr': lr_, 'scheduler': scheduler, 'scale': scale})})

    for a in [0., 1e-5, 2e-5, 5e-5, 1e-4, 2e-4, 5e-4, 1e-3, 2e-3, 5e-3, 1e-2, 2e-2, 5e-2]:
        for lr_ in [1e-3, 2e-3, 5e-3, 1e-2, 2e-2, 5e-2, 1e-1, 2e-1, 5e-1, 1e0]:
    ...
    param_dict = {
        ...
        FullBatchInnerOptimization.task_family: {
            ...
            "input_dim": input_dim,
            "device": device,
            "model_task": ModelInitialization(),
            "dtype": dtype
        },
